...
rawEEGFile = 'C:/Users/Janna/Documents/UNI/Methods/BIDS_EEG/subject1.bdf';  % Input BioSemi EEG file
rootDir = 'C:/Users/Janna/Documents/UNI/Methods/BIDS_EEG';
subjectID = 'sub-01'; % put in subject ID according to that which is given in the Raw data 
eegDir = fullfile(rootDir, subjectID, 'eeg');
outputFile = fullfile(eegDir, 'sub-01_task-rest_eeg.edf');  % Output BIDS format

% Create BIDS directories
mkdir(eegDir);

%Convert raw eeg data into BIDS format using EEGLAB application 
% Read BioSemi EEG data and save in a different format
EEG = pop_biosig('subject1.bdf');  % Read BDF file
pop_writeeeg(EEG, outputFile, 'TYPE', 'EDF');  % Save in EDF format
...
savejson('', metadata, 'C:/Users/Janna/Documents/UNI/Methods/BIDS_EEG/sub-01/sub-01_eeg.json'); %pathway where json file should be saved
